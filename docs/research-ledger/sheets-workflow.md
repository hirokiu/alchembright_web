...
`HEAD_COMMIT`は実際の参照コミットに置き換える。出力は`{format_version:1,tabs:{シート名:[[見出し],[データ行],...]}}`。使い方タブ以外の8タブを含み、日付や課題番号等は文字列。空欄はnull、継続中はTRUE/FALSEに対応する。順序列により配列順を保存する。
...
1. 対象ブランチの最新状態を取得し、シートの管理情報・全8データタブを読む。初期13件の範囲に固定せず、現在のグリッド・表の範囲を確認して分割取得する。取得前後のDrive更新日時を比較し、読取り中に編集された場合は全体を再取得する。
