...
export async function GET({site}: {site: URL | undefined}) {
 const xml=[...(await routes()), ...projects.map(p=>({path:`/works/${p.slug}/`})), {path:'/works/kirokun/'}, {path:'/works/balog/'}, {path:'/tags/'}].map(r=>`<url><loc>${new URL(r.path,site).href.replaceAll('&','&amp;')}</loc></url>`).join('');
